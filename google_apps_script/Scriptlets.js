...
var registeredMacros = ['dates','passengers','drivelink','food_rate_full','food_rate_incomplete','hotel_rate'];
...
function food_rate_full()
{
  return readFromDeductionsTable('Food_24');
}

function food_rate_incomplete()
{
  return readFromDeductionsTable('Food_8');
}

function hotel_rate()
{
  return readFromDeductionsTable('Hotel');
}

function readFromDeductionsTable(rate_name)
{
  var region_id = replacePlaceholders(('/**/Deduction**/')); //read choosed travel region id (Deduction PullDownMenu)  either from form object or from the persistent version of it (a spreadsheet )
  var sheetname = 'Deductions '+ createDateFromGerman(getDataFromXMLDoc('hinreise@datum',xmlDoc)).getFullYear();
  
  var db = objDB.open(deductions_table.getId());
